...
// NO function isAdult (personDetails) {
//   let ages = [];
...
function isMale (personGrades) {
return personGrades.sex === 'M';
}
...
function filterGradeC (array,callback) {
  let final = [];
  for(let person of array) {
    if(callback(person)) {
      final.push(person);
...
//ABOVE IS A HIGHER ORDER FUNCTION
